# 🖐🏽 Olá, sou o Wanderson Rodrigues!

**Software Engineer | .NET Specialist**

Sou um entusiasta do ecossistema .NET, focado em construir aplicações robustas e escaláveis. Meu objetivo no GitHub é explorar novas tecnologias e aplicar padrões de arquitetura modernos.

## 🛠️ O que você vai encontrar por aqui:
- **Clean Architecture & DDD:** Projetos focados em separação de preocupações e domínio rico.
- **Ecossistema Blazor:** Experimentos com Blazor WebAssembly e MAUI Hybrid para soluções multiplataforma.
- **Exploração de Microsserviços:** Implementações de mensageria (RabbitMQ/Kafka) e cache distribuído com Redis.
- **Fundamentos Sólidos:** Repositórios dedicados ao estudo profundo de C#, .NET 8 e boas práticas de OOP.

## 💻 Tech Stack:
- **Linguagens:** C#, TypeScript, SQL.
- **Backend:** .NET 8, ASP.NET Core, Entity Framework Core, NestJS.
- **Frontend/Mobile:** Blazor, .NET MAUI.
- **Ferramentas:** Docker, xUnit, Azure DevOps.

## 🌐 Conecte-se comigo:
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/wanderson-rodriguesp/)

---

## 📊 Estatísticas:

![](https://github-readme-stats.vercel.app/api?username=Wandersonrp&theme=dark&hide_border=false&include_all_commits=false&count_private=false)<br/>

![](https://github-readme-streak-stats.herokuapp.com/?user=Wandersonrp&theme=dark&hide_border=false)<br/>

![](https://github-readme-stats.vercel.app/api/top-langs/?username=Wandersonrp&theme=dark&hide_border=false&include_all_commits=false&count_private=false&layout=compact)
